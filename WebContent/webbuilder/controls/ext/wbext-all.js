...
Ext.extend(Ext.tree.TreeNode, Ext.data.Node, {
			...
			setIconClass : function(c) {
				var e = this.getUI().getIconEl();
				if (e) {
					var l = Ext.get(e);
					if (l) {
						l.removeClass(this.attributes.iconCls);
						l.addClass(c);
						this.attributes.iconCls = c;
					}
				}
			},
...
Ext.form.DateField = Ext.extend(Ext.form.TriggerField, {
	...
	onTriggerClick : function() {
		...
		var u = document.body.clientHeight, v = document.body.clientWidth, h = this.menu
				.getEl().getHeight(), m = this.menu.getEl().getWidth(), p = this.el
				.getY(), l = this.el.getX();
		if (v - l < m + 2)
			l = v - m - 2;
		if (u - p < h + 25 && p < h + 5)
			this.menu.showAt([l, 2]);
		else
 ...
 ****/
Ext.wv.DatetimePicker = function(config){
	/** Call superclass constructor **/
	Ext.wv.DatetimePicker.superclass.constructor.call(this, config);
};

Ext.extend(Ext.wv.DatetimePicker, Ext.DatePicker, {
...
/** Class Name: Ext.wv.DatetimeItem
 * Inherits From: Ext.menu.Adapter
 * Contains: Ext.wv.DatetimePicker
 * Purpose: Effectively overrides Ext.menu.DateItem so that it contains Ext.wv.DatetimePicker instead of Ext.DatePicker
 * Note: ORIGINAL and NEW comments are used to denote what differs from Ext.menu.DateItem
 */
Ext.wv.DatetimeItem = function(config){
	// ORIGINAL:
	//Ext.menu.DateItem.superclass.constructor.call(this, new Ext.DatePicker(config), config);
	// NEW:
	Ext.wv.DatetimeItem.superclass.constructor.call(this, new Ext.wv.DatetimePicker(config), config);
	// END NEW
	this.picker = this.component;
...
Ext.extend(Ext.wv.DatetimeItem, Ext.menu.Adapter, {
	onSelect : function(picker, date){
		this.fireEvent("select", this, date, picker);
		// ORIGINAL:
		//Ext.menu.DateItem.superclass.handleClick.call(this);
		// NEW:
		Ext.wv.DatetimeItem.superclass.handleClick.call(this);
		// END NEW
	}
});


/** Class Name: Ext.wv.DatetimeMenu
 * Inherits From: Ext.menu.Menu
 * Contains: Ext.wv.DatetimeItem
 * Purpose: Effectively overrides Ext.menu.DateMenu so that it contains Ext.wv.DatetimeItem instead of Ext.menu.DateItem
 * Note: ORIGINAL and NEW comments are used to denote what differs from Ext.menu.DateMenu
 */
Ext.wv.DatetimeMenu = function(config){
	// ORIGINAL:
	//Ext.menu.DateMenu.superclass.constructor.call(this, config);
	//this.plain = true;
	//var di = new Ext.menu.DateItem(config);
	// NEW:
	Ext.wv.DatetimeMenu.superclass.constructor.call(this, config);
	this.plain = true;
	var di = new Ext.wv.DatetimeItem(config);
	// END NEW
	this.add(di);
	this.picker = di.picker;
	this.relayEvents(di, ["select"]);
};
Ext.extend(Ext.wv.DatetimeMenu, Ext.menu.Menu);

/** 
 * 扩展Ext.form.DateField,增加,时,分,秒; 
 */  
Ext.wv.DatetimeField = Ext.extend(Ext.form.DateField, {  
    initComponent: function (){  
        Ext.wv.DatetimeField.superclass.initComponent.apply(this , arguments);  
        this.menu = new Ext.wv.DatetimeMenu();  
    },  
      
    //覆盖其他父类方法     
    onRender: function (){    
        Ext.wv.DatetimeField.superclass.onRender.apply(this , arguments);    
    }     
}
...
/*  测试日期控件*/
Ext.reg("datefield", Ext.wv.DatetimeField )//form.DateField);
	...
	initTemplates : function() {
		...
		if (!c.hcell) {
			c.hcell = new Ext.Template(
					'<td class="x-grid3-hd x-grid3-cell x-grid3-td-{id} {css}" style="{style} text-align:center"><div {tooltip} {attr} class="x-grid3-hd-inner x-grid3-hd-{id}" unselectable="on" style="{istyle}">',
...
Ext.grid.RowNumberer.prototype = {
	...
	renderer : function(b, c, a, d) {
		...
		var y, u = a.store.lastOptions.params;
		if (u)
			y = u.start || 0;
		else
			y = 0;
...
if (!Array.prototype.map) {
	Array.prototype.map = function(a) {
		...
		for (var i = 0; i < b; i++) {
			if (i in this) {
				c[i] = a.call(d, this[i], i, this)
			}
		}
		return c
	}
}
Ext.data.PagingMemoryProxy = function(a) {
	Ext.data.PagingMemoryProxy.superclass.constructor.call(this);
	this.data = a
};
Ext.extend(Ext.data.PagingMemoryProxy, Ext.data.MemoryProxy, {
			load : function(c, d, f, g, h) {
				c = c || {};
				var i;
				try {
					i = d.readRecords(this.data)
				} catch (e) {
					this.fireEvent("loadexception", this, h, null, e);
					f.call(g, null, h, false);
					return
				}
				if (c.filter !== undefined) {
					i.records = i.records.filter(function(a) {
								if (typeof(a) == "object") {
									var b = c.filterCol || 0;
									return String(a.data[b]).match(c.filter)
											? true
											: false
								} else {
									return String(a).match(c.filter)
											? true
											: false
								}
							});
					i.totalRecords = i.records.length
				}
				if (c.sort !== undefined) {
					var j = String(c.dir).toUpperCase() == "DESC" ? -1 : 1;
					var k = function(a, b) {
					...
					i.records.sort(function(a, b) {
								var v = 0;
								if (typeof(a) == "object") {
									v = k(a.data[c.sort], b.data[c.sort]) * j
								} else {
									v = k(a, b) * j
								}
								if (v == 0) {
...
if (Ext.util.Format) {
	Ext.util.Format.date = function(v, a) {
		if (!v)
			return "";
		if (!(v instanceof Date))
			v = new Date(Date.parse(v));
		return v.dateFormat(a || "Y-n-j")
	}
}
if (Ext.DatePicker) {
	Ext.apply(Ext.DatePicker.prototype, {
				...
				nextText : '下月 (Ctrl + 右键)',
				prevText : '上月 (Ctrl + 左键)',
				monthYearText : '选择一个月 (Ctrl + 上键/下键 来改变年)',
				todayTip : "{0} (空格键选择)",
				format : "Y-n-j",
				okText : "确定",
				cancelText : "取消"
...
if (Ext.form.HtmlEditor) {
	Ext.apply(Ext.form.HtmlEditor.prototype, {
				createLinkText : "请输入链接的URL地址:",
				fontFamilies : ["宋体", "黑体", "隶书", "华文新魏", "Arial",
...
if (Ext.form.DateField) {
	Ext.apply(Ext.form.DateField.prototype, {
				...
				invalidText : "{0} 是无效的日期，有效格式如2008-8-15",
				format : "Y-n-j"
			})
}
if (Ext.form.TimeField) {
	Ext.apply(Ext.form.TimeField.prototype, {
				...
				invalidText : "{0} 是无效的时间，有效格式如08:30:05",
				format : "H:i:s"
			})
}
