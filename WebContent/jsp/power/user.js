...
Ext.onReady(function(){
	...
	var UuserObj = [
		...
		{ name:'state', type:'int'},
		{ name:'dept_cod', type:'string'},
		{ name:'bz', type:'string'}
	];
	
	var store = new Ext.data.JsonStore({
	    url: rootpath()+'user_findPageUser.do',
	    root: 'root',
	    totalProperty: 'total',
    ...
    var grid = new Ext.grid.GridPanel({
        ...
        cm: new Ext.grid.ColumnModel({
			...
			columns:[new Ext.grid.RowNumberer(),
	            ...
	            {header: '部门', width: 150, align:'center',dataIndex: 'dept_cod'},
        	...
        	handler: function(){
				...
				if(!record){
					...
					Ext.MessageBox.confirm('删除提示', '是否删除该用户？', function(c) {
					   if(c=='yes'){
					   		Ext.Ajax.request({
					   			url : rootpath()+"user_deleteUser.do",
    ...
    var roleStore = new Ext.data.JsonStore({
	    url: rootpath()+'role_findRoleType.do',
	    root: 'root',
	    totalProperty: 'total',
	    fields : ["value", "text"]
	});
			...
			handler : function() {
				if (uForm.getForm().isValid()) {
					uForm.getForm().submit({
						url : rootpath()+'user_saveOrUpdateUser.do',
