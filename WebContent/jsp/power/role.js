...
Ext.onReady(function(){
	...
	var store = new Ext.data.JsonStore({
	    url: rootpath()+'role_findPageRole.do',
	    root: 'root',
	    totalProperty: 'total',
    ...
    var grid = new Ext.grid.GridPanel({
        store: store,
      	columns : [new Ext.grid.RowNumberer(), {
			...
			menuDisabled : true,
            items: [{
                icon   : rootpath()+'img/btn/edit2.gif',  // Use a URL in the icon config
                tooltip: '权限设置',
                handler: function(grid, rowIndex, colIndex) {
                    var record = store.getAt(rowIndex);
                    var roleid = record.get('roleid');
                	roleMenuWindow.show(null,function(){
						tree.loader.dataUrl = rootpath()+'role_findRoleMenu.do?roleid='+ roleid
        	...
        	handler: function(){
				...
				if(!record){
					...
					Ext.MessageBox.confirm('删除提示', '是否删除该角色？', function(c) {
					   if(c=='yes'){
					   		Ext.Ajax.request({
					   			url : rootpath()+"role_deleteRole.do",
			...
			handler : function() {
				if (roleForm.getForm().isValid()) {
					roleForm.getForm().submit({
						url : rootpath()+'role_saveOrUpdateRole.do',
	...
	var roleMenuWindow = new Ext.Window({
		...
		buttons : [{
			...
			handler : function() {
				...
				Ext.Ajax.request({
		   			url : rootpath()+"role_saveRoleMenu.do",
		   			params:{ 
