...
Ext.onReady(function(){
	...
	var store = new Ext.data.JsonStore({
	    url: rootpath()+'spxx_findPageSpxx.do',
	    root: 'root',
	    totalProperty: 'total',
	    fields: SpxxObj,
        	...
        	handler: function(){
				...
				if(!record){
					...
					Ext.MessageBox.confirm('删除提示', '是否删除该商品？', function(c) {
					   if(c=='yes'){
					   		Ext.Ajax.request({
					   			url : rootpath()+"spxx_deleteSpxx.do",
    ...
    var spdwStore = new Ext.data.JsonStore({
	    xtype:'jsonstore',
		url: rootpath()+'spdw_findAllSpdw.do',
	    root: 'root',
	    totalProperty: 'total',
	...
	var submitSpxx = function(next){
		if (addForm.getForm().isValid()) {
			addForm.getForm().submit({
				url : rootpath()+'spxx_saveOrUpdateSpxx.do',
	...
	var tree = new Ext.tree.TreePanel({
        ...
        frame:true,
        dataUrl: rootpath()+'splb_findSplbTree.do',
        root: {
            nodeType: 'async',
            text: '所有类别',
            draggable: false,
        	...
        	handler:function(){
        		if(v_lbid){
					...
					Ext.MessageBox.confirm('删除提示', '是否删除"'+node.text+'"类别？', function(c) {
					   if(c=='yes'){
					   		Ext.Ajax.request({
					   			url : rootpath()+"splb_deleteSplb.do",
					   			params:{ lbid : v_lbid },
					   			success : function(o) {
			...
			handler : function() {
				if (splbForm.getForm().isValid()) {
					splbForm.getForm().submit({
						url : rootpath()+'splb_saveOrUpdateSplb.do',
						params:{pid : v_lbid},
						success : function(form, action) {
							splbWindow.hide();
					...
					handler:function(){
						...
						if(!record){
							...
							Ext.MessageBox.confirm('删除提示', '是否删除该单位？', function(c) {
							   if(c=='yes'){
							   		Ext.Ajax.request({
							   			url : rootpath()+"spdw_deleteSpdw.do",
	...
	var SpdwAddWin = new Ext.Window({
		...
		fbar:[{
				...
				handler:function(){
					...
					if (form.isValid()) {
							form.submit({
								url : rootpath()+'spdw_saveOrUpdateSpdw.do',
								success : function(form, action) {
									SpdwAddWin.hide();
									spdwStore.load();
								},
	...
	var getCode = function(){
		Ext.Ajax.request({
   			url : rootpath()+"spxx_getSpxxCode.do",
   			success : function(o) {
   				if(o.responseText){
