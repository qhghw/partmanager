...
public class BillCargoAction {
	public void geSql(HttpServletRequest request, HttpServletResponse response)
		...
		if (!StringUtil.isEmpty(wtf)) {
			sql.append("  and sc.PAYER_COD = '" + wtf + "'");
		}
		if (!StringUtil.isEmpty(begin)) {
			begin = begin.substring(0, 10);
			sql.append("  and  to_char(LEAV_PORT_TIM,'YYYY-MM-DD') >= '"
					+ begin + "'");
		}

		if (!StringUtil.isEmpty(end)) {
			end = end.substring(0, 10);
			sql.append("  and  to_char(LEAV_PORT_TIM,'YYYY-MM-DD') <= '" + end
					+ "'");
		}
		if (!StringUtil.isEmpty(PM_COD)) {
			sql.append("  and  sb.TENTANTID ='" + PM_COD + "'");
		}
